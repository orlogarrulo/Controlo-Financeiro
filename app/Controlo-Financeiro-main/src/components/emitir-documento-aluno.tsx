...
export function EmitirDocumentoAluno({ alunos, onEmitted }: Props) {
  ...
  async function emitir(opts: { comPdf: boolean; comoRecibo?: boolean }) {
    if (!aluno) {
    ...
    try {
      ...
      const comoRecibo = Boolean(opts.comoRecibo);
      const numero = nextNumeroModelo(modelo, mesKey, [...documentosAluno]);
      const stamp = comoRecibo
        ? garantirCodigoReciboAluno({
            alunoId: aluno.id,
            alunoNome: aluno.nome,
            mesKey,
            valor: total,
            rubricas: activas.map((l) => l.label).join(", "),
          })
        : null;
      addDocumentoAluno?.({
        tipo: comoRecibo ? "recibo" : "fatura",
        modelo,
        numero,
        alunoId: aluno.id,
        alunoNome: aluno.nome,
        ...
        valor: total,
        linhas: activas,
        estado: comoRecibo ? "emitido" : "por_enviar",
        codigoVerificacao: stamp?.codigo,
        pagoEm: comoRecibo ? new Date().toISOString().slice(0, 10) : undefined,
        criadoPor: active,
      });
      if (opts.comPdf) {
        const html = buildInvoiceHtml({
          ...
          valor: total,
          mesRef: mesKey,
          mesLetivo: mesKey.slice(5, 7) === "10" ? "out" : "out",
          pagoMes: comoRecibo ? total : 0,
          contacto: loadContacto(),
          linhas: activas.map((l) => ({ key: l.key, label: l.label, value: l.value, on: true })),
          modo: comoRecibo ? "recibo" : "fatura",
          codigoVerificacao: stamp?.codigo,
          viaLabel: stamp?.viaLabel,
        });
        const fname = `${comoRecibo ? "recibo" : "fatura"}-${numero.replace(/[^\w\-]/g, "_")}.pdf`;
      ...
      toast.success(
        comoRecibo
          ? `Recibo ${numero} · ${formatKz(total)} · código ${stamp?.codigo || ""}`
          : `Fatura ${numero} emitida · ${formatKz(total)} · Arquivo (por enviar)`,
      );
      setLinhas([]);
      setOpen(false);
      onEmitted?.();
  ...
  return (
    <div className="w-full space-y-3 rounded-[var(--radius-md)] border border-[var(--color-forest)]/30 bg-[var(--color-card)] p-4">
      ...
      <div className="flex flex-wrap gap-2">
        <Button disabled={busy} onClick={() => void emitir({ comPdf: true })}>
          Emitir fatura + PDF
        </Button>
        <Button disabled={busy} variant="outline" onClick={() => void emitir({ comPdf: false })}>
          Só registar no Arquivo
        </Button>
        <Button
          disabled={busy}
          variant="secondary"
          onClick={() => void emitir({ comPdf: true, comoRecibo: true })}
        >
          Emitir recibo + código
        </Button>
      </div>
    </div>
  );
