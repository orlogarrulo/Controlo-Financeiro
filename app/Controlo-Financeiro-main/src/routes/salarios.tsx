...
/** Código de verificação único para recibos de honorários / adiantamento. */
function novoCodigoReciboHonorario(prefix: "RH" | "RAD", mesKey: string): string {
  const ym = (mesKey || new Date().toISOString().slice(0, 7)).replace(/-/g, "");
  const alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
  let token = "";
  for (let i = 0; i < 4; i++) token += alphabet[Math.floor(Math.random() * alphabet.length)];
  const n = String(10 + Math.floor(Math.random() * 90));
  return `${prefix}-${ym}-${token}-${n}`;
}
...
function reciboHonorarioHtml(
  ...
  const logo = escolaLogoSrc();
  const isAdiant = r.tipo === "adiantamento";
  ...
  const titulo = isAdiant
    ? "Recibo de adiantamento de honorários / salários"
    : "Recibo de honorários / prestação de serviços";
  const corpo = isAdiant
    ? `Pagámos a <b>${r.nome}</b> a quantia de <b>${formatKz(r.liquido)}</b> a título de <b>adiantamento</b> de honorários / salários referente a ${descricao}, período <b>${ini}</b> a <b>${fim}</b>. Este valor será descontado no recibo de honorários do mês.`
    : `Pagámos a <b>${r.nome}</b> a quantia de <b>${formatKz(r.liquido)}</b> referente a ${descricao}, durante o período <b>${ini}</b> a <b>${fim}</b>.`;
  const codigo = r.codigoVerificacao || "";
  ...
  <p class="tx">${corpo}</p>
  <table class="tb">
    <tr><td>${isAdiant ? "Honorário de referência (mês)" : "Honorário de referência"}</td><td class="n">${formatKz(r.salarioBruto)}</td></tr>
    ${!isAdiant && r.descontoDias > 0 ? `<tr><td>Desconto dias (${r.diasTrab}/${r.diasUteis})</td><td class="n">−${formatKz(r.descontoDias)}</td></tr>` : ""}
    ${!isAdiant && (r.outrosDesc || 0) > 0 ? `<tr><td>Outros descontos</td><td class="n">−${formatKz(r.outrosDesc)}</td></tr>` : ""}
    ${isAdiant ? `<tr><td>Valor do adiantamento</td><td class="n">${formatKz(r.liquido)}</td></tr>` : ""}
    <tr class="tot"><td>${isAdiant ? "Adiantamento pago" : "Líquido"}</td><td class="n">${formatKz(r.liquido)}</td></tr>
  </table>
  ${r.iban ? `<p class="mu">IBAN: ${r.iban}</p>` : ""}
  ${r.notas ? `<p class="mu">${escapeHtml(r.notas)}</p>` : ""}
  ${
    codigo
      ? `<p class="mu" style="margin-top:10px;letter-spacing:0.08em;text-transform:uppercase;font-weight:700;">Código de verificação</p>
  <p style="margin:4px 0 0;font-size:13px;font-weight:800;font-family:ui-monospace,Menlo,monospace;letter-spacing:0.06em;">${escapeHtml(codigo)}</p>`
      : ""
  }
  <div class="sg">
  ...
  <p class="ft">Documento gerado pelo Departamento de Finanças, ${dataDoc}${codigo ? ` · Verificar: ${escapeHtml(codigo)}` : ""}</p>
</article>`;
}
...
function Salarios() {
  ...
  const [adiantNotas, setAdiantNotas] = useState("");
  const [adiantMesKey, setAdiantMesKey] = useState("");
  const [adiantMes, setAdiantMes] = useState("");
  ...
  function gerarRecibos() {
    ...
    for (const id of selected) {
      ...
      created.push({
        ...
        tipo: "honorario",
        codigoVerificacao: novoCodigoReciboHonorario("RH", genMesKey),
      });
    }
  ...
  function openAdiantamento() {
    ...
    const comp = mesCompetenciaPagamento();
    const mesKey = filterMes && filterMes !== "todos" ? filterMes : comp.key;
    const fromOpts = opcoesMesReferencia().find((o) => o.key === mesKey);
    setAdiantMesKey(mesKey);
    setAdiantMes(fromOpts?.label || comp.label);
    setAdiantData(todayIso());
    setAdiantFuncId(rows[0]?.id || "");
    setAdiantValor(rows[0]?.salario ? String(Math.round(rows[0].salario / 2)) : "");
    setAdiantNotas("");
    setAdiantOpen(true);
  }

  function gerarAdiantamento() {
    const f = rows.find((r) => r.id === adiantFuncId);
    if (!f) {
      toast.error("Seleccione o funcionário.");
      return;
    }
    const valor = Number(String(adiantValor).replace(",", ".")) || 0;
    if (valor <= 0) {
      toast.error("Indique o valor do adiantamento.");
      return;
    }
    const existentes = (recibosSalario || []).filter(
      (r) => r.tipo === "adiantamento" && r.mesKey === adiantMesKey,
    ).length;
    const rec: ReciboSalario = {
      id: `RAD-${adiantMesKey}-${String(existentes + 1).padStart(3, "0")}`,
      funcionarioId: f.id,
      nome: f.nome,
      funcao: f.funcao,
      mes: adiantMes,
      ...
      criadoEm: new Date().toISOString(),
      tipo: "adiantamento",
      codigoVerificacao: novoCodigoReciboHonorario("RAD", adiantMesKey),
      notas: adiantNotas.trim() || undefined,
    };
    addRecibosSalario([rec]);
    setFilterMes(adiantMesKey);
    setAdiantOpen(false);
    toast.success(`Adiantamento ${rec.id} · ${formatKz(valor)} · código ${rec.codigoVerificacao}`);
    verDocumento(`Adiantamento — ${f.nome}`, wrapReciboPage(escola, rec));
  }
          ...
          <div className="no-print flex flex-row flex-wrap items-center gap-2">
            {canEdit ? (
              <>
                ...
                <Button className="shrink-0" type="button" variant="secondary" onClick={openAdiantamento}>
      ...
      <Dialog open={adiantOpen} onOpenChange={setAdiantOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="grid gap-3">
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (f) setAdiantValor(String(Math.round((f.salario || 0) / 2)));
                }}
              >
                {rows.map((r) => (
                  <option key={r.id} value={r.id}>
                    {r.nome} · {formatKz(r.salario)}
                  </option>
                ))}
              </select>
            </div>
            <div className="space-y-1">
              <Label>Mês a que o adiantamento se refere</Label>
              <select
                className="flex h-11 w-full rounded-[var(--radius-sm)] border border-[var(--color-line-strong)] bg-[var(--color-surface)] px-3 text-sm"
                value={adiantMesKey}
                onChange={(e) => {
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <div className="space-y-1">
                <Label>Valor do adiantamento (Kz)</Label>
                <Input
                  type="number"
                  value={adiantValor}
            ...
            <div className="space-y-1">
              ...
              <Input
                value={adiantNotas}
                onChange={(e) => setAdiantNotas(e.target.value)}
                placeholder="Ex.: 50% do mês, urgência familiar…"
              />
            </div>
            <p className="text-[11px] text-[var(--color-muted)]">
              O recibo sai com código de verificação. O valor deve ser descontado no honorário do mês.
            </p>
            <div className="flex justify-end gap-2 border-t pt-3">
              <Button type="button" variant="secondary" onClick={() => setAdiantOpen(false)}>
                Cancelar
              </Button>
              <Button type="button" onClick={gerarAdiantamento}>
                Emitir recibo
              </Button>
            </div>
          </div>
